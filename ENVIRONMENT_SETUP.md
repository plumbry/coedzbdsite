...
| `YUNITE_API_KEY` | `convex/yunite/*` (sync, debug, platforms, matchData, fixPlacements, populateTeamMembers, checkSurvivalTimeData, lookupPlatform), `convex/scrimSeries/importFromYunite.ts` | API key for Yunite (Discord tournament/stats integration). | **Required** for Yunite sync features. |
