...
| **Cache / refresh** | `/admin/features` (Refresh All Stats), `/admin/data-cache-status`, actions on tier-re-evaluation | Same rebuild operations in multiple places | One **Data Health** panel with per-cache status + single “Rebuild all” |
